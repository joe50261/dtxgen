...
拖入一個混音檔(mp3/ogg/wav/m4a/flac/opus)→ 開始製譜 → 自動下載譜包 zip。
...
- 跳過分離(快很多):一次拖入**兩道已分離的 stem** — 鼓軌 + 去鼓 BGM
  (例:`*.drums.opus` + `*.bgm_d.opus`,或 demucs two-stems 的 `drums` +
  `no_drums`)。依檔名自動判別哪道是鼓軌(含 `drums`、非 `no_drums`),
  另一道即去鼓 BGM。demucs stem 本是兩道 → 只給鼓軌會缺伴奏,故不接受單一鼓軌
  (單檔一律視為混音、走完整分離)
